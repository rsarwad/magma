  ...
  uint32_t len)
{
   ...
   */
  if (
    (decoded_result = decode_u8_eps_attach_type(
       &attach_request->epsattachtype,
       0,
       *(buffer + decoded) & 0x0f,
       len - decoded)) < 0) {
  ...
  if (
    (decoded_result = decode_u8_nas_key_set_identifier(
       &attach_request->naskeysetidentifier,
       0,
       *(buffer + decoded) >> 4,
       len - decoded)) < 0) {
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, TLV_VALUE_DOESNT_MATCH);
  ...
  if (
    (decoded_result = decode_ue_network_capability(
       &attach_request->uenetworkcapability,
       0,
       buffer + decoded,
       len - decoded)) < 0) {
    OAILOG_FUNC_RETURN(LOG_NAS_EMM, TLV_VALUE_DOESNT_MATCH);
  ...
  if (
    (decoded_result = decode_esm_message_container(
  ...
  *(buffer + encoded) =
  ...
  if (
    (encode_result = encode_eps_mobile_identity(
       &attach_request->oldgutiorimsi, 0, buffer + encoded, len - encoded)) <
  ...
  if (
    (encode_result = encode_esm_message_container(
       attach_request->esmmessagecontainer,
       0,
       buffer + encoded,
       len - encoded)) < 0) //Return in case of error
  ...
  if (
    ...
    ATTACH_REQUEST_VOICE_DOMAIN_PREFERENCE_AND_UE_USAGE_SETTING_PRESENT) {
    if (
      (encode_result = encode_voice_domain_preference_and_ue_usage_setting(
         &attach_request->voicedomainpreferenceandueusagesetting,
         true,
         buffer + encoded,
         len - encoded)) < 0)
