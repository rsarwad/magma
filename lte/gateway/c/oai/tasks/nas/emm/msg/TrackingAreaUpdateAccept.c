  ...
  uint32_t len)
{
   ...
   */
  if (
    (decoded_result = decode_u8_eps_update_result(
  ...
  uint32_t len)
{
  ...
  CHECK_PDU_POINTER_AND_LENGTH_ENCODER(
    buffer, TRACKING_AREA_UPDATE_ACCEPT_MINIMUM_LENGTH, len);
  //This is incoorect. No need to <<4 as it is already taken care in encode_u8_eps_update_result(). Hence commenting it
