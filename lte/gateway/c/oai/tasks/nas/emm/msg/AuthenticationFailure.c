  ...
  uint32_t len)
{
  ...
  if (
    (decoded_result = decode_emm_cause(
       &authentication_failure->emmcause, 0, buffer + decoded, len - decoded)) <
    0)
  ...
  while (len - decoded > 0) {
    ...
    switch (ieiDecoded) {
      case AUTHENTICATION_FAILURE_AUTHENTICATION_FAILURE_PARAMETER_IEI:
        if (
          (decoded_result = decode_authentication_failure_parameter_ie(
  ...
  uint32_t len)
{
  ...
  if (
    (encode_result = encode_emm_cause(
       &authentication_failure->emmcause, 0, buffer + encoded, len - encoded)) <
  ...
  if (
    ...
    AUTHENTICATION_FAILURE_AUTHENTICATION_FAILURE_PARAMETER_PRESENT) {
    if (
      (encode_result = encode_authentication_failure_parameter_ie(
         authentication_failure->authenticationfailureparameter,
         AUTHENTICATION_FAILURE_AUTHENTICATION_FAILURE_PARAMETER_IEI,
         buffer + encoded,
         len - encoded)) < 0)
