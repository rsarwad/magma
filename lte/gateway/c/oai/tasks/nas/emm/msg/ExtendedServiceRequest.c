  ...
  uint32_t len)
{
   ...
   */
  if (
    (decoded_result = decode_u8_service_type(
       &extended_service_request->servicetype,
       0,
       *(buffer + decoded) & 0x0f,
       len - decoded)) < 0)
  ...
  if (
    (decoded_result = decode_mobile_identity_ie(
       &extended_service_request->mtmsi, 0, buffer + decoded, len - decoded)) <
    0)
    return decoded_result;
  else
   ...
   * The UE shall include this IE only if the Service type information element indicates
   * "mobile terminating CS fallback or 1xCS fallback"
  */
  if (extended_service_request->servicetype == MT_CS_FB) {
    if (
      (decoded_result = decode_csfb_response(
         &extended_service_request->csfbresponse,
         0,
         (buffer + decoded),
         len - decoded)) < 0) {
      return decoded_result;
  ...
  *(buffer + encoded) =
  ...
  if (
    (encode_result = encode_mobile_identity_ie(
       &extended_service_request->mtmsi, 0, buffer + encoded, len - encoded)) <
  ...
  if (
    (encode_result = encode_csfb_response(
       &extended_service_request->csfbresponse,
       0,
       buffer + encoded,
