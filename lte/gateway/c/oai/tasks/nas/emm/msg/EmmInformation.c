  ...
  while (len - decoded > 0) {
    ...
    switch (ieiDecoded) {
      case EMM_INFORMATION_FULL_NAME_FOR_NETWORK_IEI:
        if (
          (decoded_result = decode_network_name_ie(
      ...
      case EMM_INFORMATION_LOCAL_TIME_ZONE_IEI:
        if (
          (decoded_result = decode_time_zone(
      ...
      case EMM_INFORMATION_UNIVERSAL_TIME_AND_LOCAL_TIME_ZONE_IEI:
        if (
          (decoded_result = decode_time_zone_and_time(
      ...
      case MM_DAYLIGHT_SAVING_TIME_IEI:
        if (
          (decoded_result = decode_daylight_saving_time_ie(
             &emm_information->networkdaylightsavingtime,
             MM_DAYLIGHT_SAVING_TIME_IEI,
             buffer + decoded,
             len - decoded)) <= 0)
          return decoded_result;
  ...
  uint32_t len)
{
  ...
  if (
    ...
    EMM_INFORMATION_FULL_NAME_FOR_NETWORK_PRESENT) {
    if (
      (encode_result = encode_network_name_ie(
         &emm_information->fullnamefornetwork,
         EMM_INFORMATION_FULL_NAME_FOR_NETWORK_IEI,
         buffer + encoded,
         len - encoded)) < 0)
  ...
  if (
    ...
    EMM_INFORMATION_LOCAL_TIME_ZONE_PRESENT) {
    if (
      (encode_result = encode_time_zone(
         &emm_information->localtimezone,
         EMM_INFORMATION_LOCAL_TIME_ZONE_IEI,
         buffer + encoded,
         len - encoded)) < 0)
  ...
  if (
    ...
    EMM_INFORMATION_UNIVERSAL_TIME_AND_LOCAL_TIME_ZONE_PRESENT) {
    if (
      (encode_result = encode_time_zone_and_time(
         &emm_information->universaltimeandlocaltimezone,
         EMM_INFORMATION_UNIVERSAL_TIME_AND_LOCAL_TIME_ZONE_IEI,
         buffer + encoded,
         len - encoded)) < 0)
  ...
  if (
    ...
    EMM_INFORMATION_NETWORK_DAYLIGHT_SAVING_TIME_PRESENT) {
    if (
      (encode_result = encode_daylight_saving_time_ie(
         &emm_information->networkdaylightsavingtime,
         MM_DAYLIGHT_SAVING_TIME_IEI,
         buffer + encoded,
         len - encoded)) < 0)
