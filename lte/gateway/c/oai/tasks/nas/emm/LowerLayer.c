  ...
  bool is_ciphered)
{
  ...
  if (
    context && ((context->sc_type == SECURITY_CTX_TYPE_FULL_NATIVE) ||
    ...
    data->sqn =
      context->dl_count
        .seq_num; //TODO AT check whether we need to increment this by one or it is already incremented after sending last NAS mssage.
