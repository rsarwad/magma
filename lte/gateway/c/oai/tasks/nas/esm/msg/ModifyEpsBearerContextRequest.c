  ...
  while (len - decoded > 0) {
    ...
    switch (ieiDecoded) {
      ...
      case MODIFY_EPS_BEARER_CONTEXT_REQUEST_TFT_IEI:
        if (
          (decoded_result = decode_traffic_flow_template_ie(
      ...
      case MODIFY_EPS_BEARER_CONTEXT_REQUEST_NEW_QOS_IEI:
        if (
          (decoded_result = decode_quality_of_service_ie(
             &modify_eps_bearer_context_request->newqos,
             true,
             buffer + decoded,
             len - decoded)) <= 0)
          return decoded_result;
      ...
      case MODIFY_EPS_BEARER_CONTEXT_REQUEST_NEGOTIATED_LLC_SAPI_IEI:
        if (
          (decoded_result = decode_llc_service_access_point_identifier_ie(
             &modify_eps_bearer_context_request->negotiatedllcsapi,
             true,
             buffer + decoded,
             len - decoded)) <= 0)
          return decoded_result;
      ...
      case MODIFY_EPS_BEARER_CONTEXT_REQUEST_PACKET_FLOW_IDENTIFIER_IEI:
        if (
          (decoded_result = decode_packet_flow_identifier_ie(
             &modify_eps_bearer_context_request->packetflowidentifier,
             true,
             buffer + decoded,
             len - decoded)) <= 0)
          return decoded_result;
  ...
  uint32_t len)
{
  ...
  if (
    ...
    MODIFY_EPS_BEARER_CONTEXT_REQUEST_TFT_PRESENT) {
    if (
      (encode_result = encode_traffic_flow_template_ie(
         &modify_eps_bearer_context_request->tft,
         TFT_ENCODE_IEI_TRUE,
         buffer + encoded,
         len - encoded)) < 0)
