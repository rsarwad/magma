  ...
  uint32_t len)
{
  ...
  if (
    (decoded_result = decode_esm_cause(
       &pdn_disconnect_reject->esmcause, 0, buffer + decoded, len - decoded)) <
    0)
      ...
      case PDN_DISCONNECT_REJECT_PROTOCOL_CONFIGURATION_OPTIONS_IEI:
        if (
          (decoded_result = decode_protocol_configuration_options_ie(
             &pdn_disconnect_reject->protocolconfigurationoptions,
             true,
             buffer + decoded,
             len - decoded)) <= 0)
          return decoded_result;
  ...
  uint32_t len)
{
  ...
  if (
    (encode_result = encode_esm_cause(
       &pdn_disconnect_reject->esmcause, 0, buffer + encoded, len - encoded)) <
