...
static nas_emm_common_proc_t *get_nas_common_procedure(
  ...
  emm_common_proc_type_t proc_type);
static nas_cn_proc_t *get_nas_cn_procedure(
  const struct emm_context_s *const ctxt,
  cn_proc_type_t proc_type);
...
{
  if (
    LIST_EMPTY(&emm_context->emm_procedures->emm_common_procs) &&
  ...
  nas_base_proc_t *const parent_proc)
{
  ...
  if (emm_context->emm_procedures) {
    ...
    if (emm_context->emm_procedures->emm_con_mngt_proc) {
      if (
        ((nas_base_proc_t *) (emm_context->emm_procedures->emm_con_mngt_proc))
          ->parent == parent_proc) {
      ...
      if (p1->proc == cn_proc) {
        switch (cn_proc->type) {
          case CN_PROC_AUTH_INFO:
            ...
            break;
          case CN_PROC_NONE: free_wrapper((void **) &cn_proc); break;
          default:;
        }
        LIST_REMOVE(p1, entries);
        free_wrapper((void **) &p1);
  ...
  if (emm_context->emm_procedures) {
    ...
    while (p1) {
      ...
      switch (p1->proc->type) {
        ...
        default: free_wrapper((void **) &p1->proc);
      }
      LIST_REMOVE(p1, entries);
      free_wrapper((void **) &p1);
      p1 = p2;
  ...
  uint64_t puid)
{
  if ((emm_context) && (emm_context->emm_procedures)) {
    ...
    if (emm_context->emm_procedures->emm_specific_proc) {
      if (
        emm_context->emm_procedures->emm_specific_proc->emm_proc.base_proc
          .nas_puid == puid) {
        OAILOG_TRACE(
    ...
    if (emm_context->emm_procedures->emm_con_mngt_proc) {
      if (
        emm_context->emm_procedures->emm_con_mngt_proc->emm_proc.base_proc
          .nas_puid == puid) {
        OAILOG_TRACE(
