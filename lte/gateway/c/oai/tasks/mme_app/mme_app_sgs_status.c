...
int mme_app_handle_sgs_status_message(
  ...
  if (ue_context_p->sgs_context == NULL) {
    OAILOG_ERROR(LOG_MME_APP, "SGS context not created for IMSI "
                 IMSI_64_FMT "\n", imsi64);
...
#define MESSAGE_TYPE_POSITION  0
  ...
  switch (message_type) {
    case SGS_LOCATION_UPDATE_REQUEST: {
      OAILOG_ERROR(LOG_MME_APP, "Received SGS Error Status message for"
    ...
    case SGS_SERVICE_REQUEST: {
      if (ue_context_p->sgs_context->csfb_service_type ==
        ...
        if (!ue_context_p->sgs_context->mt_call_in_progress) {
          ...
          OAILOG_INFO(LOG_MME_APP, "Can not abort MT call, as MT call is"
    ...
    default: {
      OAILOG_ERROR(LOG_MME_APP, "Received unknown messag type in SGS Error"
...
static void _mme_app_handle_sgs_status_for_eps_detach_ind(
  ...
  if (ue_context_p->sgs_context) {
    ...
    */
    if (ue_context_p->sgs_context->ts8_timer.id !=
        MME_APP_TIMER_INACTIVE_ID) {
      if (timer_remove(ue_context_p->sgs_context->ts8_timer.id, NULL)) {
        OAILOG_ERROR (LOG_MME_APP, "Failed to stop"
