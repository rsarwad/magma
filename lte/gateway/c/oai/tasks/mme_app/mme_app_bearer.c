    ...
    if (is_guti_valid) {
      ...
      if (ue_nas_ctx) {
        ...
        if (ue_context_p != NULL) {
           ...
           * When UE move to IDLE this key is set to INVALID.
           * Note - This can happen if eNB detects RLF late and
           * by that time UE sends Initial NAS message via new RRC
           * connection
  ...
  itti_s11_create_session_response_t *const create_sess_resp_pP)
{
  ...
  if (create_sess_resp_pP->cause.cause_value != REQUEST_ACCEPTED) {
    ...
    OAILOG_ERROR(
      LOG_MME_APP, "Handling NAS PDN connectivity failure for ue_id = (%u)"
      ", bearer id = (%d)"
      ", pti = (%d)\n",
      ue_context_p->mme_ue_s1ap_id,
      bearer_id,
  ...
  itti_mme_app_initial_context_setup_rsp_t *const initial_ctxt_setup_rsp_pP)
{
  ...
  OAILOG_INFO(
    LOG_MME_APP, "Received MME_APP_INITIAL_CONTEXT_SETUP_RSP"
    "from S1AP for ue_id =" MME_UE_S1AP_ID_FMT "\n",
  ...
  const itti_s11_create_bearer_request_t *const create_bearer_request_pP)
{
  ...
  for (int i = 0;
    ...
    activate_ded_bearer_req = (emm_cn_activate_dedicated_bearer_req_t){0};
    ...
    if(activate_ded_bearer_req.pco ) {
      free(activate_ded_bearer_req.pco);
    }
  }
  OAILOG_FUNC_OUT(LOG_MME_APP);
}
  ...
  uint8_t failed_procedure)
{
  ...
  OAILOG_INFO(
    ...
    "Notify Service Reject to NAS for ue_id :" MME_UE_S1AP_ID_FMT "\n",
    ue_id);
  ...
  const ebi_t ebi)
{
  ...
  if (ue_context_p == NULL) {
    OAILOG_ERROR(
      LOG_MME_APP,
      "Failed to find UE context, while sending create dedicated bearer"
      " response for ue-id:" MME_UE_S1AP_ID_FMT "\n",ue_id);
    OAILOG_FUNC_OUT(LOG_MME_APP);
  }
  ...
  const ebi_t ebi)
{
  ...
  if (ue_context_p == NULL) {
    OAILOG_ERROR(
      LOG_MME_APP,
      "Failed to find UE context, while sending create dedicated bearer"
      " reject for ue-id:" MME_UE_S1AP_ID_FMT "\n",ue_id);
    OAILOG_FUNC_OUT(LOG_MME_APP);
  }
  ...
  *const nw_init_bearer_actv_req_p)
{
  ...
  OAILOG_INFO(
    LOG_MME_APP,
    "Handling MME_APP_CREATE_DEDICATED_BEARER_REQ with UE ID"
    MME_UE_S1AP_ID_FMT "for LBI %d\n",
    activate_ded_bearer_req.ue_id,
    ue_context_p->pdn_contexts[cid]->default_ebi);
  ...
  bstring nas_msg)
{
  ...
  if (bearer_context) {
    ...
    OAILOG_INFO(
      LOG_MME_APP,
      "Sending ERAB REL CMD to S1AP with ue-id" MME_UE_S1AP_ID_FMT
      " and EBI %u \n", ue_id, ebi);
    itti_send_msg_to_task(TASK_S1AP, INSTANCE_DEFAULT, message_p);
  } else {
    OAILOG_ERROR(
      LOG_MME_APP,
      "No bearer context found ue-id " MME_UE_S1AP_ID_FMT " ebi %u\n",
      ue_id,
      ebi);
  }
  OAILOG_FUNC_OUT(LOG_MME_APP);
...
{
  struct ue_mm_context_s *ue_context_p = NULL;
  uint32_t no_of_bearers = 1;
  ebi_t ebi_list[BEARERS_PER_UE] = {0};
  ...
  if (ue_context_p == NULL) {
    OAILOG_ERROR(
      LOG_MME_APP,
      "Failed to find UE context while sending Dedicated Bearer Deactivation"
      " Resp message for ue-id: " MME_UE_S1AP_ID_FMT "\n", ue_id);
    OAILOG_FUNC_OUT(LOG_MME_APP);
  }

  //Send delete_dedicated_bearer_rsp to SPGW
  ebi_list[0] = ebi;
  _send_delete_dedicated_bearer_rsp(ue_context_p, delete_default_bearer,
     ebi_list, no_of_bearers, s_gw_teid_s11_s4, REQUEST_ACCEPTED);
  ...
  teid_t s_gw_teid_s11_s4)
{
  ...
  if (ue_context_p == NULL) {
    OAILOG_ERROR(
      LOG_MME_APP,
      "Failed to find UE context while sending Dedicated Bearer Deactivation"
      " Rej message for ue-id: " MME_UE_S1AP_ID_FMT "\n", ue_id);
    OAILOG_FUNC_OUT(LOG_MME_APP);
  }
