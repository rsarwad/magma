 ...
 * @param detach_type - SGS EPS detach type
 */
  ...
  uint8_t detach_type)
{
  ...
  memset(
    (void *) &message_p->ittiMsg.sgsap_eps_detach_ind,
    0,
  ...
  if (detach_type == SGS_NW_INITIATED_IMSI_DETACH_FROM_EPS) {
    cb.nas_timer_callback = mme_app_handle_sgs_implicit_eps_detach_timer_expiry;
  } else {
    cb.nas_timer_callback = mme_app_handle_sgs_eps_detach_timer_expiry;
  }
  ...
  if (detach_type == SGS_NW_INITIATED_IMSI_DETACH_FROM_EPS) {
    if (timer_setup(
      ...
      OAILOG_ERROR(
        LOG_MME_APP,
        "Failed to start SGS Implicit EPS Detach indication timer for UE id  "
        "%d \n",
        ue_context_p->mme_ue_s1ap_id);
      ...
      OAILOG_DEBUG(
        LOG_MME_APP,
        "Started SGS Implicit EPS Detach indication timer for UE id  %d \n",
        ue_context_p->mme_ue_s1ap_id);
    }
  } else {
    ...
    if (timer_setup(
      ...
      OAILOG_ERROR(
        LOG_MME_APP,
        "Failed to start SGS EPS Detach indication timer for UE id  %d \n",
        ue_context_p->mme_ue_s1ap_id);
      ue_context_p->sgs_context->ts8_timer.id = MME_APP_TIMER_INACTIVE_ID;
    } else {
      OAILOG_DEBUG(
        LOG_MME_APP,
        "Started SGS EPS Detach indication timer for UE id  %d \n",
        ue_context_p->mme_ue_s1ap_id);
    }
...
void mme_app_handle_sgs_eps_detach_timer_expiry(void *args)
{
  ...
  if (ue_context_p == NULL) {
    OAILOG_ERROR(
      LOG_MME_APP,
      "SGS EPS Detach Timer expired but no assoicated UE context for UE "
  ...
  if (
    ...
    mme_app_send_sgs_eps_detach_indication(
      ue_context_p, ue_context_p->detach_type);
  } else {
    OAILOG_DEBUG(
      LOG_MME_APP,
...
void mme_app_handle_sgs_implicit_eps_detach_timer_expiry(void *args)
{
  ...
  if (ue_context_p == NULL) {
    OAILOG_ERROR(
      LOG_MME_APP,
      "SGS IMPLICIT EPS DETACH Timer expired but no assoicated UE context for"
 ...
 if (ue_context_p->sgs_context == NULL) {
  ...
  if (
    ...
    OAILOG_DEBUG(
      ...
      ue_context_p->sgs_context->ts13_retransmission_count);
    increment_counter(
      "sgs_eps_implicit_detach_timer_expired",
      1,
      1,
      "cause",
  ...
  uint8_t detach_type)
{
  ...
  AssertFatal(message_p, "itti_alloc_new_message Failed");
  memset(
    (void *) &message_p->ittiMsg.sgsap_imsi_detach_ind,
    0,
  ...
  if (detach_type == SGS_IMPLICIT_NW_INITIATED_IMSI_DETACH_FROM_EPS_N_NONEPS) {
    cb.nas_timer_callback =
      mme_app_handle_sgs_implicit_imsi_detach_timer_expiry;
  } else {
    cb.nas_timer_callback  = mme_app_handle_sgs_imsi_detach_timer_expiry;
  }
  cb.nas_timer_callback_arg = (void *) &(ue_context_p->mme_ue_s1ap_id);
  if (detach_type == SGS_IMPLICIT_NW_INITIATED_IMSI_DETACH_FROM_EPS_N_NONEPS) {
    ...
    if (timer_setup(
      ...
      &cb,
      sizeof(cb),
      &(ue_context_p->sgs_context->ts9_timer.id)) < 0) {
      OAILOG_ERROR(
        LOG_MME_APP,
...
void mme_app_handle_sgs_imsi_detach_timer_expiry(void *args)
{
  ...
  if (ue_context_p == NULL) {
    OAILOG_ERROR(
      LOG_MME_APP,
      "SGS IMSI Detach Timer expired but no assoicated UE context for UE "
      "id " MME_UE_S1AP_ID_FMT "\n",
      mme_ue_s1ap_id);
    OAILOG_FUNC_OUT(LOG_MME_APP);
  }
...
void mme_app_handle_sgs_implicit_imsi_detach_timer_expiry(void *args)
{
  ...
  if (ue_context_p == NULL) {
    OAILOG_ERROR(
      LOG_MME_APP,
      "SGS IMPLICIT IMSI Detach Timer expired but no assoicated UE context for UE "
      "id " MME_UE_S1AP_ID_FMT "\n",
      mme_ue_s1ap_id);
    OAILOG_FUNC_OUT(LOG_MME_APP);
  }
  ...
  if (
    ...
    OAILOG_DEBUG(
      LOG_MME_APP,
      "SGS Implicit IMSI DETACH indication failed after %u retransmission and "
      "expiry \n",
      ue_context_p->sgs_context->ts10_retransmission_count);
    increment_counter(
      "sgs_imsi_implicit_detach_timer_expired",
      1,
      1,
      "cause",
  ...
  if (ue_context_p->sgs_context) {
    ...
    if (ue_context_p->sgs_context->ts9_timer.id != MME_APP_TIMER_INACTIVE_ID) {
      if (timer_remove(ue_context_p->sgs_context->ts9_timer.id, NULL)) {
        OAILOG_ERROR(
          LOG_MME_APP,
          "Failed to stop SGS IMSI Detach Indication timer for UE id %d \n",
          ue_context_p->mme_ue_s1ap_id);
      }
      ue_context_p->sgs_context->ts9_timer.id = MME_APP_TIMER_INACTIVE_ID;
    }
